...
**Хотфикси след първия run (доставени, зелени — поправят реалното поведение):**
1. **Глобално `.hidden` правило (routing видимост)** — renderRoute слагаше клас `hidden` на `.tab-nav`/`.tab`, но styles.css имаше САМО скоупнати правила (`.map-preview.hidden`, `#baseDetail.hidden`) → табовете и списъкът не се криеха и детайлът „лепваше" ОТДОЛУ под видимия списък. Fix: глобален utility `.hidden { display: none !important; }` в края на styles.css (`!important` бие `.tab.active`, което иначе печели по специфичност). Странично излекувано: `#btnInstall` (клас hidden) също никога не се е криел реално. Unit тестовете НЕ хванаха бъга (jsdom не смята CSS, а те проверяваха classList) — затова: (а) характеризация в `bases-foundation.spec.js`, че глобалното правило съществува с `!important`; (б) нов e2e `bases-routing.spec.js` (4) + `bases-routing-fixture.html`, която НАРОЧНО линква РЕАЛНИЯ `/styles.css` (не инлайн копие като другите фикстури) — toBeVisible/toBeHidden четат computed styles и хващат липсващо CSS правило.
2. **Без deep link — остатъчен hash се чисти при зареждане (решение на потребителя)** — hash-ът оцеляваше F5 и route-ът възстановяваше детайла; потребителят иска reload и навигация от друг таб ВИНАГИ да кацат на таблицата. Fix в `base-detail.js`: при module init остатъчен `#base/<id>` се маха с `history.replaceState` (без нов history запис, без spurious hashchange); клонът „unknown id" също ползва replaceState и пада към табовете без bail (deep-link изчакването е махнато). Детайлът е достъпен САМО през 📖 в рамките на сесията. Тестът „deep link показва детайла" е ОБЪРНАТ: „stale hash се чисти при boot".
3. **Потвърждение на записване ВЪРХУ бутона (решение на потребителя — БЕЗ toast/snackbar)** — „Запази" не даваше видима реакция (syncMsg е дребен и незабележим на телефон). `saveBaseDetail` кара САМИЯ `#btnBaseSave` да говори: „Запазване…" (disabled, докато `saveBases` е in-flight) → „✓ Записано" + клас `btn-saved` (зелено, `--success`) за 1.6s → връща се „Запази"; при грешка текстът се връща веднага и грешката се препредава. Timer-ът се clear-ва при повторен save (бързи последователни записи не мигат). Стилове: `#btnBaseSave:disabled` / `#btnBaseSave.btn-saved` в styles.css. Toast/snackbar НАРОЧНО няма — потребителят го отхвърли като лош mobile UX; потвърждението е под палеца, върху контрола, който е натиснат.

**Test state (фаза 4 — доставено, всичко зелено):**
- Unit (Vitest): `bases-foundation.spec.js` (7 — таб/markup/данни/wiring + .hidden характеризацията, 610+хотфикс), `bases.spec.js` (15 — списък/модал/акордеон/drag, 620), `bases-detail.spec.js` (7 — routing + детайл; deep-link тестът обърнат от хотфикс 2, 630), `bases-tables.spec.js` (11 — под-таблиците, 640). Общо unit spec-ове след фаза 4: **15 файла** (11 от фаза 3 + 4 bases).
...
**Данни (ГОТОВИ — не се генерират от агент):** `modules/crafting-data.js` е КОМИТНАТ, генериран ES модул (~79KB): `export const CRAFTING_TABLES = [...]` — **11 таблици** (от 2026-09-01, третият източник `DnD_Raw_Materials_Economy.xlsx` добави `materials` — Суровини, 120 реда, badgeCol **'Rarity'** (единствената ИСТИНСКА rarity колона), filterable, и `rarityRules` — Рядкост (правила), 6 реда, без филтър): `{ key, label, group, type: 'table'|'info', nameCol, badgeCol (или null), filterable, columns, rows: [{col: value}] }`; info таблиците имат `entries: [{k, v}]` вместо rows. НЕ се редактира на ръка (регенерация: `tools/gen-crafting-data.cjs`). НИКАКЪВ Firestore — данните са статични (нула reads, решение на потребителя). В останалите таблици rarity ролята играе `badgeCol` (Size/Potion Tier/Property); филтърът е per таблица и се крие при `filterable: false`. UI-ят е изцяло data-driven — нова таблица = нов chip даром; чупят се само закованите бройки в спековете (foundation: 11).

**Routing рефактор (фундамент 710):** нов `modules/router.js` = ЕДИНСТВЕНИЯТ hashchange владетел:
- `dispatchRoute()`: вика `renderRoute()` (base-detail, владее `#base/<id>` и tab view) и СЛЕД това `renderCraftingRoute()` (crafting, владее `#crafting`) — последният печели, така `#crafting` скрива каквото base-detail е показал.
- `window.addEventListener('hashchange', dispatchRoute)` + boot strip: `if (location.hash) history.replaceState(null, '', location.pathname + location.search)` — ВСЕКИ остатъчен hash (и `#base/*`, и `#crafting`) се чисти при зареждане (решението от bases хотфикс 2: F5 винаги каца на табовете).
- `base-detail.js` ГУБИ собствения си `hashchange` listener и boot-strip блок (местят се в router.js), но ПАЗИ `renderRoute` export-а и btnBaseBack/btnBaseSave wiring-а. Тестовете на bases остават зелени (те викат renderRoute директно или dispatch-ват hashchange — router-ът е импортнат от app.js и слуша).
- app.js: `import './modules/router.js'` + `dispatchRoute()` след wiring (или router-ът се самоинициализира при import — избери едното и го документирай в кода). Snapshot-ът на BASES_DOC продължава да вика `renderRoute()` (не dispatchRoute — crafting не зависи от Firestore данни).

**DOM контракт (целият markup във фундамента 710):**
- Header реда (до Export/Import): `<button class="btn-ghost" onclick="openCrafting()" aria-label="Крафтинг референции">⚒</button>` — `openCrafting()` сетва `location.hash = '#crafting'` + вика dispatch-а (в crafting.js, stub в 710).
- `<div id="craftingPage" class="hidden">` СЛЕД `#baseDetail`: бутон `btnCraftBack` „← Назад" (btn-ghost); `.craft-controls`: `<input id="craftSearch" type="search" placeholder="Търси по име…">` + `<select id="craftBadge"></select>`; `<nav id="craftTabs" class="craft-tabs"></nav>` (chips, пълни се от crafting.js); `.tbl-wrap` с таблица `craftTable` (`thead id="craftHead"`, `tbody id="craftBody"`); `<div id="craftInfo" class="hidden"></div>` (за type:'info').
- styles.css: `.craft-tabs` — хоризонтално скролируема chips лента (overflow-x auto, gap, chip = btn-ghost стил с `.active` акцент); `.craft-controls` (flex, търсачката расте, на телефон wrap); `.craft-details` — key:value блок в expanded ред (label muted + стойност, НЕ таблица); `tr.craft-expanded`; `.craft-badge` (span бадж като .badge).

**Модули (контрактът между паралелните lanes):**

| Модул | Lane | Съдържание |
|-------|------|-----------|
| `modules/crafting.js` | crafting-ui | `renderCraftingRoute()` — `#crafting` → скрий `.tab-nav`/`.tab`/`#baseDetail` (клас hidden), покажи `#craftingPage`, извикай `renderCrafting()`; друг hash → скрий `#craftingPage`. `openCrafting()` — сетва hash + рендер. `renderCrafting()` — chips по CRAFTING_TABLES (active = `state.craftingTab`; клик: сетва craftingTab, нулира `state.craftingFilter` до `{q:'', badge:''}` и `state.expandedCraftIdx`, dispatch `document`-event **`crafting-tab`**, re-render); активната таблица: type 'info' → `#craftInfo` (k/v списък), таблицата скрита; type 'table' → thead (Име | badgeCol ако има | 📖), tbody по филтъра (`q` = case-insensitive substring на nameCol; `badge` = точно съвпадение на badgeCol), name в `<strong>`, badge в `.craft-badge`, акордеон през `state.expandedCraftIdx` (ред-клик И 📖 toggle-ват; expanded редът показва `.craft-details` блок с ВСИЧКИ останали колони като key: value, БЕЗ празни стойности); празен резултат → ред `.empty` „Няма съвпадения.". Слуша `crafting-filter` → `renderCrafting()`. btnCraftBack wiring (hash = '' + dispatch). БЕЗ Sortable, БЕЗ редакция — статична референция |
| `modules/crafting-search.js` | crafting-search | Wiring на `#craftSearch` (`input` event) и `#craftBadge` (`change`): пишат `state.craftingFilter = {q, badge}` + dispatch `document`-event **`crafting-filter`**. `populateBadgeOptions()`: за `state.craftingTab` → ако `filterable` → опции „Всички" (value '') + distinct badgeCol стойности по ред на поява, селектът видим; иначе клас hidden. Слуша `crafting-tab` → чисти input/select + populate. НЕ вика crafting.js директно (паралелна lane — само state + events) |

**state.js добавя:** `craftingTab: 'animals', craftingFilter: { q: '', badge: '' }, expandedCraftIdx: null`.
...
**Добавка след първия run (таск 910 — чист test-only, БЕЗ промяна в поведение или код):** третият xlsx източник (`materials`/`rarityRules`, виж по-горе) пристигна с данни и UI „даром" от data-driven-ия рендер, но без закована test характеризация — 910 я затваря. Нови тестове (изведени от самите данни, не memorised бройки — регенерация с повече редове не чупи спека): `crafting-foundation.spec.js` — схема на `materials` (120 реда, `Raw Material`/**`Rarity`**, filterable) + anchor ред 0 (`Common soil`/`Common`) + схема на `rarityRules` (6 реда, без badge филтър); `crafting-search.spec.js` — badge селектът на `materials` показва distinct Rarity стойности по ред на поява (`Rare` ≠ `Very Rare`, отделни опции), `rarityRules` (filterable:false) крие селекта; `crafting.spec.js` — нов `describe('crafting — raw materials economy')`: chip рендерира 120-те реда с `Rarity` badge, акордеон детайлите показват economy колоните (Category, Typical Refined Product / Notes) БЕЗ да повтарят nameCol/badgeCol, badge филтърът е ТОЧНО съвпадение (`Rare` не тегли `Very Rare`), `rarityRules` рендерира 6 реда с двуколонен head без badge.

**Test state (фаза 5, вкл. таск 910 — доставено, всичко зелено):**
- Unit (Vitest): `crafting-foundation.spec.js` (13 — икона/markup/router/данни вкл. materials/rarityRules, 710+910), `crafting.spec.js` (23 — chips/списък/акордеон/info изглед вкл. raw materials economy describe, 720+910), `crafting-search.spec.js` (12 — търсене+badge филтър вкл. materials/rarityRules, 730+910). Общо unit spec-ове: **18 файла**, **234 теста**, `npm run test:unit` зелен.
- E2e (Playwright): `test/e2e/crafting-routing.spec.js` (7) + `crafting-fixture.html` (740, реалния styles.css) — непроменени от 910 (test/e2e и fixtures не се пипат, §3.5; 910 е unit-only). Съществуващите items/quests/maps/bases e2e непроменени.
- Доставката следва плана 1:1 — БЕЗ хотфикс рунд (за разлика от maps §9 и bases §10); 910 е доп. характеризация, не хотфикс.
