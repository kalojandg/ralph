...
### СПИРАНЕ И РЕСТАРТ НА ГОЛЯМ BOARD (безопасно по конструкция)

**НЕ трий worktree-тата — те СЕ ПРЕИЗПОЛЗВАТ.** Процедура за пауза:
1. Ctrl+C на оркестратора;
2. Затвори агентските прозорци (те са отделни процеси и си продължават иначе!);
3. Рестарт: `START-RALPH-SWARM.bat N` — оркестраторът сам отказва да тръгне, ако има
   живи агенти от старата сесия (orphan guard, казва кои pids).

Какво се случва при рестарта:
- **Merged таскове** (passes:true) — не се пипат, прогресът е в tasks.json;
- **Прекъснати в движение** — worktree + branch са запазени → CONTINUATION: новият агент
  получава „INTERRUPTED RUN" бележка + наследява цялото състояние (комитнато И
  некомитнато) и продължава, не преправя;
- **Провалени от старата сесия** — броячите се нулират, continuation с пълната история;
- Claims се чистят автоматично; quota паузи не губят нищо.
Т.е. голям board се кара на порции спокойно: спри вечерта, пусни сутринта.
