...
- **Редове** = № на място (`SeatDefinitions.SeatNumber`), само физически присъстващи (`IsPhysicallyPresent = 1`). ⚠️ **ВСИЧКИ физически места са редове, не само заетите** — свободно място присъства с празни клетки. Източникът на данни ТРЯБВА да emit-не пълната решетка (места × сегменти), anchor-нат на `SeatDefinitions`, не на `SeatAvailability` (иначе се показват само продадените и заетостта излиза 100%). Виж задача 264.
