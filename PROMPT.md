...
> 🎯 **Task-Specific Steps:** Ако най-долу в този prompt има секция **"Task-Specific Steps"** — тя е инжектирана от харнеса САМО за текущия таск (от `task-steps.json`). Тези стъпки са **ЗАДЪЛЖИТЕЛНИ** и се изпълняват на мястото, посочено в полето `at` (напр. `before-DONE`, `after-8.3`). BLOCKING hook = таскът НЕ е готов и не се маркира `passes:true` докато hook-ът не мине успешно. Ако секцията липсва — просто следвай steps от tasks.json нормално.
